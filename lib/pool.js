...
var GwMaker = require('./gwMake.js');
var blockMake = require('./blockMake.js');
var getArr = global.getArr = [];
...
var pool = module.exports = function pool(options, authorizeFn){
            ...
            DetectCoinData(function(){
                ...
                OnBlockchainSynced(function(){
                    GetFirstJob(function(){
                        SetupBlockPolling();
                       if(options.usc.enabled){ SetupNewUscWorkBroadcast(); }
                        SetupPeer();
    ...
    */
    function SubmitBlock(blockHex, callback){
        ...
        _this.daemon.cmd(rpcCommand,
            ...
            function(results){
                ...
                emitLog('Submitted Ulord Block using ' + rpcCommand + ' successfully to daemon instance(s)');
                callback();
            }
            );
	...
	// USC SubmitBlock
    function SubmitUscBlock(params) {
        var blockM = new blockMake(options.usc);
	//if(_this.nSubmitted < 5) {
		_this.nSubmitted++;
		emitLog("USC Submitted: " + _this.nSubmitted);
		blockM.makeSubmitMsg(params, function(err, rawbm){
			if(rawbm === undefined && err !== null) {
				emitErrorLog('Failed to submit potential usc block: ' + JSON.stringify(err));
				return;
			} else {
				_this.nSubmitted--;
				emitLog('Submitted usc block successfully to the USC daemon instance(s) with BlockHash: ' + rawbm.blockHash);
			}
		});
	//}
    }
    ...
    function SetupJobManager(){
        ...
        _this.jobManager.on('newBlock', function(blockTemplate){
            ...
            if(options.usc.enabled){
            if(isValidShare && !shareData.isDuplicate){
                if(shareData.isUscTarget){

                    if(shareData.blockHash === undefined){
                        var blockHashHex = shareData.blockHashInvalid;
                    }else{
                        var blockHashHex = shareData.blockHash;
                    }
                    var params = [
                        blockHashHex, //blockHashHex
                        shareData.headerBuffer, //blockHeaderHex
                        shareData.coinbaseTxHex, //coinbaseHex
                        shareData.merkleHashes, //merkleHashesHex
                        shareData.cTotalTxCountHex  //blockTxnCountHex
                    ]

                    SubmitUscBlock(params);
	    	    }
            }
	    }
...
function SetupNewUscWorkBroadcast(){
    var gwMaker = new GwMaker(options.usc);
    if(process.env.forkId==0){
        var pollingInterval = options.blockRefreshInterval;
        blockPollingIntervalId = setInterval(function () {
            gwMaker.makeRawGwMsg(function(rawgw){
                if(rawgw !== undefined){
                    if(!rawgw.notify){
                        if(getArr.indexOf(rawgw.blockHashForMergedMining) === -1){
                            getArr.push(rawgw.blockHashForMergedMining);
                            data.isDuplicate = false;
                            _this.stratumServer.emit('broadcastTimeout');
                        }else{
            ...
            if (result.error){
                ...
                if(options.usc.enabled){
                    ...
                    gwMaker.makeRawGwMsg(function(rawgw){
                        ...
                        var processedNewBlock = _this.jobManager.processTemplate(data);
